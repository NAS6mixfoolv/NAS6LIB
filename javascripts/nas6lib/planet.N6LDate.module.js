...
//planet//惑星
class N6LPlanetN6LDate {

  constructor(rh) {

    this.typename = "N6LPlanetN6LDate";
    this.m_earth;             //earth//地球

    this.m_pno;               //planet no.//惑星番号
    this.m_pname;             //planet name//惑星名前
    this.m_dat0 = new N6LDate(); //datetime//日時
    ...
    if(rh != undefined && rh.typename == "N6LPlanetN6LDate") {
      this.typename = rh.typename;
      this.m_earth = rh.m_earth;
      this.m_pno = rh.m_pno;
      this.m_pname = rh.m_pname;
      this.m_dat0 = new N6LDate(rh.m_dat0);
    ...
    // Assign an independent bit to indicate which N6LVector is different as the return value of Comp/EpsComp of N6LPlanetN6LDate
    // ネストされたオブジェクトの差異 (子オブジェクトの比較結果を直接利用、または専用フラグ)
    // N6LVectorのComp/EpsCompの返り値をそのままANDして使うことも可能ですが、
    // N6LPlanetN6LDateのComp/EpsCompの戻り値としてどのN6LVectorが異なるかを表す独立したビットを割り当てます
    ...
    Comp(rh) {
        var ret = 0;
        var i;
        if(rh.typename == "N6LPlanetN6LDate"){
            if(this.m_pno !== rh.m_pno) ret |= N6LPlanetN6LDate.DIFF_PNO;
            if(this.m_pname !== rh.m_pname) ret |= N6LPlanetN6LDate.DIFF_PNAME;
            if(this.m_dat0.toUNIX() !== rh.m_dat0.toUNIX()) ret |= N6LPlanetN6LDate.DIFF_DAT0;
            if(this.m_nday !== rh.m_nday) ret |= N6LPlanetN6LDate.DIFF_NDAY;
            if(this.m_a !== rh.m_a) ret |= N6LPlanetN6LDate.DIFF_A;
            if(this.m_e !== rh.m_e) ret |= N6LPlanetN6LDate.DIFF_E;
            if(this.m_l0 !== rh.m_l0) ret |= N6LPlanetN6LDate.DIFF_L0;
            if(this.m_nperday !== rh.m_nperday) ret |= N6LPlanetN6LDate.DIFF_NPERDAY;
            if(this.m_ra !== rh.m_ra) ret |= N6LPlanetN6LDate.DIFF_RA;
            if(this.m_rb !== rh.m_rb) ret |= N6LPlanetN6LDate.DIFF_RB;
            if(this.m_t !== rh.m_t) ret |= N6LPlanetN6LDate.DIFF_T;
            if(this.m_s !== rh.m_s) ret |= N6LPlanetN6LDate.DIFF_S;
            if(this.m_i !== rh.m_i) ret |= N6LPlanetN6LDate.DIFF_I;
            if(this.m_w !== rh.m_w) ret |= N6LPlanetN6LDate.DIFF_W;
            if(this.m_mv !== rh.m_mv) ret |= N6LPlanetN6LDate.DIFF_MV;
            if(this.m_m !== rh.m_m) ret |= N6LPlanetN6LDate.DIFF_M;
            if(this.m_r !== rh.m_r) ret |= N6LPlanetN6LDate.DIFF_R;
            if(this.x0 !== rh.x0) ret |= N6LPlanetN6LDate.DIFF_X0;
            if(this.v0 !== rh.v0) ret |= N6LPlanetN6LDate.DIFF_V0;
            if(this.ex !== rh.ex) ret |= N6LPlanetN6LDate.DIFF_EX;
            if(this.m_el !== rh.m_el) ret |= N6LPlanetN6LDate.DIFF_EL;
            if(this.m_d !== rh.m_d) ret |= N6LPlanetN6LDate.DIFF_D;
            if(this.m_asc !== rh.m_asc) ret |= N6LPlanetN6LDate.DIFF_ASC;
            if(this.m_hs.length !== rh.m_hs.length) ret |= N6LPlanetN6LDate.DIFF_HS_LENGTH;
            if(this.m_rev !== rh.m_rev) ret |= N6LPlanetN6LDate.DIFF_REV;
            if(this.CNST_G !== rh.CNST_G) ret |= N6LPlanetN6LDate.DIFF_CNST_G;
            if(this.CNST_C !== rh.CNST_C) ret |= N6LPlanetN6LDate.DIFF_CNST_C;
            if(this.CNST_AU !== rh.CNST_AU) ret |= N6LPlanetN6LDate.DIFF_CNST_AU;
            if(this.CNST_DR !== rh.CNST_DR) ret |= N6LPlanetN6LDate.DIFF_CNST_DR;
            if(this.CNST_TAU !== rh.CNST_TAU) ret |= N6LPlanetN6LDate.DIFF_CNST_TAU;
        }
        else ret |= N6LPlanetN6LDate.DIFF_TYPE;
        return ret;
    };
    ...
    EpsComp(rh, eps) {
       if(!eps) eps = 1e-6;
        ...
        if(rh.typename == "N6LPlanetN6LDate"){
            if(this.m_pno !== rh.m_pno) ret |= N6LPlanetN6LDate.DIFF_PNO;
            if(this.m_pname !== rh.m_pname) ret |= N6LPlanetN6LDate.DIFF_PNAME;
            if(Math.abs(this.m_dat0.toUNIX() - rh.m_dat0.toUNIX()) > eps) ret |= N6LPlanetN6LDate.DIFF_DAT0;
            if(this.m_nday < rh.m_nday - eps || rh.m_nday + eps < this.m_nday) ret |= N6LPlanetN6LDate.DIFF_NDAY;
            if(this.m_a < rh.m_a - eps || rh.m_a + eps < this.m_a) ret |= N6LPlanetN6LDate.DIFF_A;
            if(this.m_e < rh.m_e - eps || rh.m_e + eps < this.m_e) ret |= N6LPlanetN6LDate.DIFF_E;
            if(this.m_l0 < rh.m_l0 - eps || rh.m_l0 + eps < this.m_l0) ret |= N6LPlanetN6LDate.DIFF_L0;
            if(this.m_nperday < rh.m_nperday - eps || rh.m_nperday + eps < this.m_nperday) ret |= N6LPlanetN6LDate.DIFF_NPERDAY;
            if(this.m_ra < rh.m_ra - eps || rh.m_ra + eps < this.m_ra) ret |= N6LPlanetN6LDate.DIFF_RA;
            if(this.m_rb < rh.m_rb - eps || rh.m_rb + eps < this.m_rb) ret |= N6LPlanetN6LDate.DIFF_RB;
            if(this.m_t < rh.m_t - eps || rh.m_t + eps < this.m_t) ret |= N6LPlanetN6LDate.DIFF_T;
            if(this.m_s < rh.m_s - eps || rh.m_s + eps < this.m_s) ret |= N6LPlanetN6LDate.DIFF_S;
            if(this.m_i < rh.m_i - eps || rh.m_i + eps < this.m_i) ret |= N6LPlanetN6LDate.DIFF_I;
            if(this.m_w < rh.m_w - eps || rh.m_w + eps < this.m_w) ret |= N6LPlanetN6LDate.DIFF_W;
            if(this.m_mv < rh.m_mv - eps || rh.m_mv + eps < this.m_mv) ret |= N6LPlanetN6LDate.DIFF_MV;
            if(this.m_m < rh.m_m - eps || rh.m_m + eps < this.m_m) ret |= N6LPlanetN6LDate.DIFF_M;
            if(this.m_r < rh.m_r - eps || rh.m_r + eps < this.m_r) ret |= N6LPlanetN6LDate.DIFF_R;
            if(!this.x0.EpsEqual(rh.x0, eps)) ret |= N6LPlanetN6LDate.DIFF_X0;
            if(!this.x0.EpsEqual(rh.v0, eps)) ret |= N6LPlanetN6LDate.DIFF_V0;
            if(!this.ex.EpsEqual(rh.ex, eps)) ret |= N6LPlanetN6LDate.DIFF_EX;
            if(this.m_el < rh.m_el - eps || rh.m_el + eps < this.m_el) ret |= N6LPlanetN6LDate.DIFF_EL;
            if(this.m_d < rh.m_d - eps || rh.m_d + eps < this.m_d) ret |= N6LPlanetN6LDate.DIFF_D;
            if(this.m_asc < rh.m_asc - eps || rh.m_asc + eps < this.m_asc) ret |= N6LPlanetN6LDate.DIFF_ASC;
            if(this.m_hs.length != rh.m_hs.length) ret |= N6LPlanetN6LDate.DIFF_HS_LENGTH;
            if(this.m_rev != rh.m_rev) ret |= N6LPlanetN6LDate.DIFF_REV;
            if(this.CNST_G != rh.CNST_G) ret |= N6LPlanetN6LDate.DIFF_CNST_G;
            if(this.CNST_C != rh.CNST_C) ret |= N6LPlanetN6LDate.DIFF_CNST_C;
    ...
    clone() {
      return new N6LPlanetN6LDate(this);
    };
    ...
    CreateMoon(pno, pname, nday, dat0, add, earth, bbb) {
        ...
        var dat1 = new N6LDate(1974, 12, 31, 0, 0, 0);
        var msecPerMinute = 1000 * 60;
        var msecPerHour = msecPerMinute * 60;
        var msecPerDay = msecPerHour * 24;
        var dat0Msec = this.m_dat0.toUNIX();
        var dat1Msec = dat1.toUNIX();
    ...
    CreateEtc(pno, pname, nday, dat0, sw, earth) {
        ...
        else if(sw == 3) { //no include asc include N6LPlanetN6LDate(i,0).m_hs(0)//asc何も入らないN6LPlanetN6LDate(i,0).m_hs(0)に入ってる
            this.m_el = 0.0;
            if(this.m_el < 0.0) this.m_el += 360.0;
            this.ex.x[0] = this.m_el;
        }
        else if(sw == 4) { //no include mc include N6LPlanetN6LDate(i,0).m_hs(9)//mc何も入らないN6LPlanetN6LDate(i,0).m_hs(9)に入ってる
            this.m_el = 0.0;
            if(this.m_el < 0.0) this.m_el += 360.0;
            this.ex.x[0] = this.m_el;
        }
...
*/
    ...
    //ascendant//アセンダント計算
    asc(dat1) {
        //var dat1 = new N6LDate(1974, 12 - 1, 31, 0, 0, 0);
        var msecPerMinute = 1000 * 60;
        var msecPerHour = msecPerMinute * 60;
        var msecPerDay = msecPerHour * 24;
        var dat0Msec = this.m_dat0.toUNIX();
        ...
        var dat = new N6LDate(1996, 7 - 1, 1, 0, 0, 0);
        this.m_asc = 150.0;
        var msecPerMinute = 1000 * 60;
        var msecPerHour = msecPerMinute * 60;
        var msecPerDay = msecPerHour * 24;
        var datMsec = dat.toUNIX();
        var dat1Msec = dat1.toUNIX();
