//Programed by NAS6
//keyboard.js
import { N6LTimer, N6LTimerMan, TMUpdate } from '../nas6lib/timer.module.js';
      ...
      for(j in UnityAliasID[i]) {
        if(UnityAliasID[i][j] == str) {
          for(k in UnityAliasID[i]) {
            if(this.keystate[this.indexof(this.ToReal(UnityAliasID[i][k]))]) return true;
          }
          return false;
        }
      }
    }
    return this.keystate[this.indexof(this.ToReal(str))];
  };
...
function initKeyBoard(tman, func) {
  ...
  if(tman) TManKeyBoard = tman.clone();
...
export { N6LKeyBoard, KeyB, initKeyBoard, loopKeyBoard };
