...
class N6LTimer {
  ...
  copy(src) {
    ...
    this.alarm = src.alarm;
    this.alarmfunc = src.alarmfunc;
  };
  now() {
    if(this.starttime != 0){
      var dt = new Date();
...
// 1. すでに別の TMUpdate が実行中なら、即座に終了して重複を防ぐ
  ...
  if (timerman.enable == true) {
    ...
    for (var m in timerman.timer) {
      ...
      if (tm.enable == true && 0 <= tm.alarm) {
        var now = tm.now();
        if (tm.alarm <= now) {
          tm.alarm = -1;
          tm.isUpdating = true;
          tm.alarmfunc(tm.ID); // GLoopなどが呼ばれる
          tm.isUpdating = false;
        }
      }
    }
...
export { N6LTimer, N6LTimerMan, TMUpdate };
