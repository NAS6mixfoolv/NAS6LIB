...
class N6LRngKtN6LDate {

  constructor() {

    this.typename = "N6LRngKtN6LDate";
...
*/
  ...
  return Math.atan2(c11,c00);
    ...
    // t (年) を秒に変換
      ...
      var a_meter = Math.cbrt((this.CNST_G * (m1 + m2) * Math.pow(t_sec, 2)) / (4 * Math.pow(Math.PI, 2)));
  
      // メートルを AU に変換
      var a = a_meter / this.CNST_AU;
      var e = 1 - (q / a);
    ...
    //2.5PN重力放射減衰
    ToGravityRadiationLoss(v, m1, m2, r, t = -1){
      //NAS6定数//解析的デバッグにより推定
      const VAL_NAS6 = 1.35 - 3.3e+8 / r;
      const CNST_NAS6 = -4.5e-1;
      //const CNST = this.CNST_C * CNST_NAS6 * VAL_NAS6;
      const CNST = 1.0;
      //var ret = (-32 * CNST / 5) * ((Math.pow(this.CNST_G, 3) * m1 * m2 * (m1 + m2))/(Math.pow(this.CNST_C, 4) * Math.pow(r, 4))) * v;
      //var ret = (-32 * CNST / 5) * ((Math.pow(this.CNST_G, 3) * m1 * m2 * (m1 + m2))/(Math.pow(this.CNST_C, 4) * Math.pow(r, 4))) * v * this.dt / t;
      var ret = (-32 * CNST / 5) * ((Math.pow(this.CNST_G, 3) * m1 * m2 * (m1 + m2))/(Math.pow(this.CNST_C, 4) * Math.pow(r, 4))) * v / t;
//      var ret = (-32 * CNST / 5) * ((Math.pow(this.CNST_G, 2) * m2 * (m1 + m2))/(Math.pow(this.CNST_C, 4) * Math.pow(r, 2))) * v;
      return ret;
    };
...
*/
    ToSchwartz(v, e, m, m0, r) {
        //var ret = (-6.0 * Math.PI * this.CNST_G * (m0 + m) /(this.CNST_C * this.CNST_C * r * (1 - e * e)));  //一般相対論
        var ret = 3.0 * v * v;//一般相対論
        return ret;
    };

    //calc accel//加速度の計算
    GetA(r, m, mr, v, e, m0 = 1, t = -1) {
        if(r == 0.0) return 0.0;
        var a = 1.0;
        if(mr <= r) mr = r;
        else a = r / mr;
...
*/
        var g = this.CNST_G * (m / mr / mr); 
        g = g * (1.0 + this.ToSchwartz(v, e, m, m0, r));
...
/*
                            if((this.blog)&&(j == 1)&&(i == 0)){
                              ...
                              var currentOrbit = this.OrbitElementCalc(q, t, m1, m2);
  ...
  */
                          var q = this.mp[i].lastQ;
                          var t = this.mp[i].lastT;
                          var ts = t * 86400 * 365.2425;
                          var m2 = this.planet[i].m_m;
                          var m1 = this.planet[j].m_m;
                          var currentOrbit = this.OrbitElementCalc(q, t, m1, m2);
                          var e_latest = currentOrbit.e; // これが「今の宇宙」の離心率
                          a1 = this.GetA(this.r[i][j], this.mp[j].mass, this.mp[j].r, ov.Abs(), e_latest, this.mp[i].mass);
                          //a1 = this.GetA(this.r[i][j], this.mp[j].mass, this.mp[j].r, ov.Abs(), 0.0, this.mp[i].mass);
                          this.pow[i][j] = a1;
                          this.l1[i][j] = this.ToGravityRadiationLoss(ov.Abs(), this.mp[j].mass, this.mp[i].mass, this.r[i][j], ts);
                          if(isNaN(this.l1[i][j])) this.l1[i][j] = 0;
                        }
...
*/
                            ...
                            var currentOrbit = this.OrbitElementCalc(q, t, m1, m2);
                            var e_latest = currentOrbit.e; // これが「今の宇宙」の離心率
                            a1 = this.GetA(this.r[i][j], this.mp[i].mass, this.mp[i].r, ov.Abs(), e_latest, this.mp[j].mass);
                            //a1 = this.GetA(this.r[i][j], this.mp[i].mass, this.mp[i].r, ov.Abs(), 0.0, this.mp[j].mass);
                            this.pow[j][i] = a1;
                            this.l1[j][i] = this.ToGravityRadiationLoss(ov.Abs(), this.mp[i].mass, this.mp[j].mass, this.r[i][j], ts);
                            if(isNaN(this.l1[j][i])) this.l1[j][i] = 0;
                        }
...
if(this.bbaphe == 0) this.bbaphe = 1;
    ...
    onPerihelion(i, lr, pos, time){
        if(this.epoch){
          var datt = this.epoch.toUNIX(); // Get the timestamp of the base date
          var dat1t = datt + this.time * 1000; // Calculate the new timestamp
          var dat1 = new N6LDate(dat1t); // Create a new Date object for the updated time
