...
class N6LDate {
  ...
  adjustTime(val){
    ...
    if (val.da !== undefined && val.da !== null) {
      var totalDay = val.da; 
      var extraMon = Math.floor((totalDay - 1) / 31);     
      if (val.ma === undefined || val.ma === null) val.ma = 1;
      val.ma += extraMon;                          
      var remainingDay = (totalDay - 1) % 31 + 1;              
      ...
      // 例：ma が 15.5 の場合
      var totalMon = val.ma; 
      var extraYear = Math.floor((totalMon - 1) / 12); // 12ヶ月分を計算 (1年)
      val.ya += extraYear;                              // 年に加算
      var remainingMon = (totalMon - 1) % 12 + 1;              
  ...
  calendarToDay(val) {
    var at = this.adjustTime(val);
    var tmp = new N6LDate(at.ya,at.ma,at.da,at.ho,at.mo,at.so,at.ms,val.region,val.regionW);
    var flg = this.afterGregorian(tmp);
    if(0 < tmp.ma && tmp.ma < 3) { tmp.ya--; tmp.ma += 12; }
    var bday = tmp.ya * 365;
    var lp = this.getLeap(tmp);
    var yd = Math.floor(306*(tmp.ma+1)/10)+(tmp.da-429);
    ...
    if(!flg) rd = 0;
    var cc = new N6LDate(jed.ya,jed.ma,jed.da,0,0,0,0,this.region,this.regionW);
    var med = this.getMonthEnd(cc,false);
    if(jed.ma == gsd.ma) med = gsd.da;
    if(12 < tmp.ma) { tmp.ya++; tmp.ma -= 12; }
    if(((tmp.ya == jed.ya && tmp.ma == jed.ma && jed.da <= tmp.da && tmp.da <= med))||
       ((tmp.ya == gsd.ya && tmp.ma == gsd.ma && 1 <= tmp.da && tmp.da <= gsd.da))) {
      tmp = cc;
      if(0 < tmp.ma && tmp.ma < 3) { tmp.ya--; tmp.ma += 12; }
      yd = Math.floor(306*(tmp.ma+1)/10)+(tmp.da-429);
    }
    return (bday + lp - rd + yd + 2);
  }
  ...
  afterGregorian(val){
    ...
    gsd = this.suboneday(gsd);
    var rd = Math.floor(reform.reformDays);
    var cc = new N6LDate(jed.ya,jed.ma,jed.da,0,0,0,0,val.region,val.regionW);
    var med = this.getMonthEnd(cc,false);
    if(jed.ma == gsd.ma) med = gsd.da;
    if((val.ya < jed.ya) ||
  ...
  isLeap(val, flg = true){
    ...
    if(Math.floor(tmp.ma) == 14 && Math.floor(tmp.da) == 29) { tmp.ya++; tmp.ma -= 11; tmp.da -= 29; }
    if(flg) return ((tmp.ya % 4 === 0)&&((tmp.ya % 100 !== 0)||(tmp.ya % 400 === 0)));
    return (tmp.ya % 4 === 0);
  }
  ...
  getLeap(val) {
    ...
    var tmp = new N6LDate(val.ya, val.ma, val.da, val.ho, val.mo, val.so, val.ms, val.region, val.regionW);
    if(0 < tmp.ma && tmp.ma < 3){
      tmp.ya--;
      tmp.ma += 12;
    }
  ...
  gregorianDayToDate(targetDay, flg = true) {
    ...
    if(0 < val.ma && val.ma < 3) { val.ya--; val.ma += 12; }
    ...
    if(val.ya == this.ya && 12 < val.ma) val.ya--;
    var lp = this.getLeap(val);
    if(0 < val.ma && val.ma < 3) { val.ya--; val.ma += 12; }
    var tdays = Math.floor(val.ya * 365) + lp - rd;

    var x = (day + 305) - tdays;
    var nl;
    var m = 3;
    if (365 < x){
      nl = (this.isLeap(val)) ? 1 : 0;
      day -= nl;
      if(x !== 366 || !nl){
        x -= 365; val.ya++;
      }
  ...
  adjustCalendar(rh = null) {
    ...
    var sourceDays = this.calendarToDay(val);
    ...
    var targetDay = sourceDays + addDays + (val.pastdays - Math.floor(val.pastdays));

    if(this.region === "SE"){//スウェーデン特殊歴
      if(!addDays) {
        // 1712-02-30 はそのまま許可
        if(val.ya === 1712 && val.ma === 2 && val.da === 30){
          val.pastdays = targetDay;
          return val;
        }
      ...
      if(val.ya >= 1700 && val.ya <= 1712){
        ...
        //addDays++;
        // 3. 目的地のトータル通算日//this.pastdays//通算チックタイム
        //targetDay = sourceDays + addDays + (val.pastdays - Math.floor(val.pastdays));
      }
    }
    ...
    var gsd = this.parseDate(reform.gregorianStart);
    var rd = Math.floor(reform.reformDays);
    var julianEndDay = this.calendarToDay(new N6LDate(jed.ya,jed.ma,jed.da,0,0,0,0,val.region,val.regionW));
    var gregorianStartDay = this.calendarToDay(new N6LDate(gsd.ya,gsd.ma,gsd.da,0,0,0,0,val.region,val.regionW));
